...
class SocialFeedManager {
    ...
    setupEventListeners() {
        ...
        // ✅ Event delegation para botões de curtir comentários e respostas
        document.addEventListener('click', (e) => {
            ...
            if (e.target.classList.contains('comment-like-btn') && e.target.dataset.replyId) {
                e.preventDefault();
                e.stopPropagation();
                ...
                if (commentElement && postElement) {
                    ...
                    const postId = postElement.dataset.postId;
                    console.log('Reply like clicked via delegation:', { postId, commentId, replyId }); // Debug
    ...
    addCommentToUI(postId, comment) {
        ...
        // ✅ Adiciona event listeners para os botões do comentário
        this.addCommentEventListeners(commentElement, postId, comment.id);
    ...
    updateCommentInUI(postId, comment) {
        ...
        if (commentElement) {
            ...
            // ✅ Re-adiciona event listeners após atualização
            this.addCommentEventListeners(newElement, postId, comment.id);
    ...
    addReplyToUI(postId, commentId, reply) {
        ...
        if (subCommentsContainer) {
            ...
            // ✅ Adiciona event listeners para os botões da resposta
            this.addReplyEventListeners(newReplyElement, postId, commentId, reply.id);
    ...
    updateReplyInUI(postId, commentId, reply) {
        ...
        if (replyElement) {
            ...
            // ✅ Re-adiciona event listeners após atualização
            this.addReplyEventListeners(newElement, postId, commentId, reply.id);
    ...
    // ✅ FUNÇÃO SIMPLIFICADA: Event listeners são gerenciados por delegation
    addCommentEventListeners(commentElement, postId, commentId) {
        // Event listeners são gerenciados por event delegation global
        // Não é mais necessário adicionar listeners individuais
        console.log('Comment added to UI:', commentId); // Debug
    }

    // ✅ FUNÇÃO SIMPLIFICADA: Event listeners são gerenciados por delegation
    addReplyEventListeners(replyElement, postId, commentId, replyId) {
        // Event listeners são gerenciados por event delegation global
        // Não é mais necessário adicionar listeners individuais
        console.log('Reply added to UI:', replyId); // Debug
    }
    ...
    addPostEventListeners(postElement, postId) {
        ...
        if (commentSubmitBtn) {
            commentSubmitBtn.addEventListener('click', () => this.submitComment(postId));
        }
