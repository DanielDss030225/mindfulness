...
class RichTextEditor {
    ...
    toggleLineSpacing() {
        ...
        if (selection.rangeCount > 0) {
            ...
            while (commonAncestor && commonAncestor !== this.editor && commonAncestor.nodeType !== 1) {
                commonAncestor = commonAncestor.parentNode;
            }

            if (commonAncestor && commonAncestor !== this.editor) {
                const currentSpacing = commonAncestor.style.lineHeight || '1';
                let newSpacing;
                ...
                commonAncestor.style.lineHeight = newSpacing;
                console.log(`Espaçamento alterado para: ${newSpacing} no elemento selecionado.`);
            } else {
                // Se não houver seleção ou a seleção for o editor inteiro, aplica a todos os filhos de bloco
                this.applyLineSpacingToChildren();
            }
        } else {
    ...
    applyLineSpacingToChildren() {
        const children = this.editor.children;
        let currentSpacing = '1'; // Assume 1 se nenhum filho tiver espaçamento definido

        // Tenta obter o espaçamento atual de um dos filhos para manter a alternância
        if (children.length > 0) {
            currentSpacing = children[0].style.lineHeight || '1';
        }
        ...
        for (let i = 0; i < children.length; i++) {
            if (children[i].nodeType === 1) { // Apenas elementos
                children[i].style.lineHeight = newSpacing;
            }
        }
        console.log(`Espaçamento alterado para: ${newSpacing} em todos os elementos de bloco.`);
    }
